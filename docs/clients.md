...
在下文中，我们可能会提供一系列关键词。您可以定位到 Log 文件中最后一串 Traceback（以许多at开头的行）的前一行，选中并复制从 `java.` 开始到最后。然后使用 Ctrl + F 快捷键进行检索，或者将关键词复制到日志文件中进行检索。请注意，原则上每次只查询一行日志，不建议同时复制和查询多行。

如果您的游戏因为未响应而崩溃，我们暂时没有办法解决 :(

## 说明及名词解释

`Forge/NeoForge/Fabric/Quilt` 以上 4 个均为 MOD 加载器，绝大多数的崩溃和 MOD 安装不正确有关。

`Optifine/高清修复` 是一个 MOD，它可以提高游戏的帧率，减少游戏的卡顿和延迟，并提供更多的图像设置。不正确的使用此 MOD 很容易导致崩溃，不建议和多个优化模组同时安装。

`JAVA` Minecraft/我的世界 启动所必须的环境。在 1.16 及之前的版本推荐使用 JAVA 8。在 1.17 及以后的版本，需要使用 JAVA 17及更高的版本。
